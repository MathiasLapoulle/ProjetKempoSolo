...
export function buildCompetitorsRouter() {
    ...
    return router.openapi(CompetitorsRoutes.get, async (ctx) => {
        ...
        .openapi(CompetitorsRoutes.importCSV, async (ctx) => {
            ...
            try {
                for (const competitorData of body.competitors) {
                    try {
                ...
                return ctx.json({
                    message: `Import terminé: ${imported} compétiteurs importés, ${failed} échecs`,
                    imported,
                    failed,
                    errors: errors.length > 0 ? errors : undefined
                }, 201)
            } catch (error) {
                return ctx.text("Erreur lors de l'import CSV", 400)
            }
        })
}